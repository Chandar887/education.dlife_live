...
if (isset($_GET['live_class_id']) && $_GET['live_class_id'] != "") {
    $live_contest_id = $_GET['live_class_id'];
}

$contstq = mysqli_query($db->con, "SELECT * FROM `live_class_data` where id = '$live_contest_id'");
?>
<div class="container-fluid">
    <main role="main" class="col-md-12 pt-3">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mb-3 border-bottom">
            <h2 class="h2"><i class="fa fa-trophy pr-2"></i>Live Class Contest Details</h2>
...
<!--                    <form method='get' action='<?php echo $currenturl; ?>' class="form-inline mr-4">
        ...
        <?php
        if (isset($request['live_class_id']) && $cData = $db->selectRow("live_class_data", "", "id = '{$request['live_class_id']}'")) {
//            $cData['category_name'] = "";
...
// echo "SELECT * FROM `quiz_score` WHERE contest_id = '$live_contest_id' AND created_at = '$created_at' ORDER BY quiz_score DESC,complete_time ASC";die;

                        $rank_q = mysqli_query($db->con, "SELECT * FROM `quiz_play_detail` WHERE live_class_id = '$live_contest_id' ORDER BY score DESC,complete_time ASC");
                                ...
                                while ($quiz_data = mysqli_fetch_array($rank_q)) {
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td class="text-right"><a href="view_quiz_details.php?user_id=<?php echo $user_id; ?>&&live_class_id=<?php echo $live_contest_id; ?>" class="btn btn-sm btn-primary"><i class="fa fa-eye"></i></a></td>
